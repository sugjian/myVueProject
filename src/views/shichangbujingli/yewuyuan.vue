<template>
  <div>
    ...
    >
      <md-field>
        <md-input-item
          title="手机号" type="phone" :readonly="isadd"
          v-model="mobile"
          placeholder="请输入手机号"
        ></md-input-item>
        <md-input-item
...
<script>
  ...
  export default {
    ...
    methods: {
      init(){
        const that=this;
        util.post('shichangbu_yewuyuan_list',{},function(datas2){
          that.list=datas2.list;          
        });
      },
      xiangqing(detail){
        this.$router.push({name:'yewuyuan_shejishi_home_view',query:{id:detail.id}});    
      },
      tianjia(){
        this.mima='';
        this.mobile='';
      ...
      onActConfirm() {
        if(!this.mobile){
         ...
         const that=this;
         if(this.isadd==false){
           util.post('mendian_shejibu_insert',{mobile:this.mobile,password:this.mima},function(datas2){
              Toast.succeed('添加成功！');
              that.actDialog.open = false;
              that.init();
            });  
         }else{
           util.post('updateuserpwd',{id:this.curdetail.id,password:this.mima},function(datas2){
              Toast.succeed('修改成功！');
              that.actDialog.open = false;
            });   
         }         
